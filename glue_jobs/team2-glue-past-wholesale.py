...
GROUP BY AUC_YMD,
        ...
        MTC_GRAD_NM,
        MTC_NM
  HAVING (SUM(CAST(UNIT_QYT * KG_UNIT_CNVR_QYT AS INT)) > 0 AND SUM(CAST(UNIT_QYT * PRCE AS INT)) = 0)
    OR (SUM(CAST(UNIT_QYT * KG_UNIT_CNVR_QYT AS INT)) = 0 AND SUM(CAST(UNIT_QYT * PRCE AS INT)) > 0)
ORDER BY AUC_YMD
""")
